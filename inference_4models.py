...
DISTILLED_CHECKPOINT = "/home/jetson/Downloads/formina/vqa_pruned_50_distilled_epoch5.pth"
...
# 4. VQA 체크포인트 로드 + Vocab
print("[*] Loading saved VQA model from:", ORIGINAL_CHECKPOINT)
t0 = time.time()
saved_state = torch.load(ORIGINAL_CHECKPOINT, map_location="cpu")
print("[*] Done loading checkpoint. Took {:.2f} sec".format(time.time() - t0))
...
# 원본 VQA 가중치 clean 버전
CLEAN_VQA_STATE = {k.replace("module.", ""): v for k, v in saved_state["weights"].items()}



# 4-1. Distilled 체크포인트도 서버 시작 시 1회 RAM에 캐싱
CLEAN_DISTILLED_STATE = None

print("[*] Preloading Distilled weights into RAM (if exists)...")
if os.path.exists(DISTILLED_CHECKPOINT):
    try:
        t1 = time.time()
        checkpoint = torch.load(DISTILLED_CHECKPOINT, map_location="cpu")

        if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
            CLEAN_DISTILLED_STATE = {
                k.replace("module.", ""): v
        ...
        print("[*] Distilled weights cached. Took {:.2f} sec".format(time.time() - t1))
    except Exception as e:
        CLEAN_DISTILLED_STATE = None
        print("[!] Distilled preload failed:", e)
else:
    print("[*] Distilled checkpoint not found. Step 3/4 will be skipped.")
...
class ResNetLayer4(nn.Module):
    def __init__(self):
        super().__init__()
        print("[*] Loading ResNet152 (pretrained=True)...")
        t0 = time.time()
        # ImageNet pretrained 로딩 후 VQA checkpoint로 덮어씀
...
def load_original_vqa_weights(model):
    safe_load_state_dict(model, CLEAN_VQA_STATE)



# 10-1. 전역 모델 (Original / Pruned) + Pruned base state
MODEL_ORIGINAL = None       # unpruned, FP32
MODEL_PRUNED = None         # layer4 structured pruning, FP32
PRUNED_BASE_STATE = None    # pruning 후 (no distill) weight snapshot (CPU)


def init_models():
    """
    서버 시작 시 한 번만 호출해서:
     - MODEL_ORIGINAL: 원본 FP32
     - MODEL_PRUNED  : layer4 50% pruning된 FP32
     - PRUNED_BASE_STATE: PRUNED 모델의 프루닝-only state_dict (CPU clone)
    Distilled weight는 매 /ask 마다 MODEL_PRUNED에 safe_load_state_dict로 적용.
    """
    global MODEL_ORIGINAL, MODEL_PRUNED, PRUNED_BASE_STATE

    print("[*] Initializing VQA models (no deepcopy, minimal count)...")

    # 1) Original FP32 (unpruned)
    print("  - Building Original FP32 model...")
    base = VQAResNetModel(num_tokens)
    load_original_vqa_weights(base)
    base.to(device).eval()
    MODEL_ORIGINAL = base
    print("  ✔ Original FP32 ready")

    # 2) Pruned (No Retraining) 모델
    print("  - Building Pruned model (50%) from scratch...")
    pruned = VQAResNetModel(num_tokens)
    load_original_vqa_weights(pruned)

    pruned = pruned.to("cpu")
    pruned_count = apply_structured_pruning(pruned.resnet_layer4.r_model, ratio=0.5)
    print(f"    [Pruning] Applied to {pruned_count} Bottleneck blocks")

    # pruning-only weight snapshot 저장 (CPU 텐서 clone)
    state_cpu = {}
    for k, v in pruned.state_dict().items():
        state_cpu[k] = v.cpu().clone()
    PRUNED_BASE_STATE = state_cpu

    pruned.to(device).eval()
    MODEL_PRUNED = pruned
    print("  ✔ Pruned model ready (base weights snapshot stored)")

    if CLEAN_DISTILLED_STATE:
        print("  Distilled state cached in RAM (will be applied per-request)")
    else:
        print("  No distilled weights. Step 3/4 will be skipped at runtime.")
...
def run_all_models_for_question(image_path, question):
    global MODEL_ORIGINAL, MODEL_PRUNED, PRUNED_BASE_STATE
    ...
    # 안전장치: 모델 초기화 안 되어 있으면 런타임 초기화
    if MODEL_ORIGINAL is None or MODEL_PRUNED is None or PRUNED_BASE_STATE is None:
        print("[WARN] Global models not initialized. Calling init_models() lazily.")
        init_models()

    # Step 1: Original FP32
    if MODEL_ORIGINAL is not None:
        print("\n--- [1] Original FP32 (cached model) ---")
        results.append(
            run_once_bench(
                MODEL_ORIGINAL,
                img,
        ...
        print("[APP] MODEL_ORIGINAL is None. Skipping Step 1.")

    # Step 2: Pruned (No Retraining) FP32
    if MODEL_PRUNED is not None and PRUNED_BASE_STATE is not None:
        print("\n--- [2] Pruned (No Retraining) (cached model + base state) ---")
        MODEL_PRUNED.float().to(device).eval()
        MODEL_PRUNED.load_state_dict(PRUNED_BASE_STATE, strict=False)
        results.append(
            run_once_bench(
                MODEL_PRUNED,
                img,
                q_tensor,
                q_len,
                "2. Pruned (No Retraining)",
            )
        )
    else:
        print("[APP] MODEL_PRUNED or PRUNED_BASE_STATE is None. Skipping Step 2.")

    # Step 3: Pruned + Distilled FP32
    if MODEL_PRUNED is not None and CLEAN_DISTILLED_STATE:
        print("\n--- [3] Pruned + Distilled (safe_load state) ---")
        MODEL_PRUNED.float().to(device).eval()
        safe_load_state_dict(MODEL_PRUNED, CLEAN_DISTILLED_STATE)
        results.append(
            run_once_bench(
                MODEL_PRUNED,
                img,
        ...
        print("[APP] MODEL_PRUNED is None or no CLEAN_DISTILLED_STATE. Skipping Step 3.")

    # Step 4: Pruned + Distilled + FP16 (CUDA only)
    if device.type == "cuda" and MODEL_PRUNED is not None and CLEAN_DISTILLED_STATE:
        print("\n--- [4] Pruned + Distilled + FP16 ---")
        MODEL_PRUNED.half().to(device).eval()
        img_fp16 = img.half()
        results.append(
            run_once_bench(
                MODEL_PRUNED,
        ...
        if device.type != "cuda":
            print("[APP] Device is not CUDA. Skipping Step 4.")
        else:
            print("[APP] MODEL_PRUNED is None or no CLEAN_DISTILLED_STATE. Skipping Step 4.")
...
def ask():
    ...
    results, final_answer = run_all_models_for_question(image_path, question)
...
if __name__ == "__main__":
    print("=" * 60)
    print("VQA Demo Server")
    print("=" * 60)
    print("✅ /capture, /upload_image: IO + base64 ONLY (NO torch, NO GPU)")
    print("✅ /ask: cached Original/Pruned models로 4-step bench (multi-run 평균+표준편차)")
    ...
    # 모델 워밍업 (서버 시작 시 1회, deepcopy 없음)
    try:
        init_models()
    except Exception as e:
        print("[ERROR] init_models failed:", e)
